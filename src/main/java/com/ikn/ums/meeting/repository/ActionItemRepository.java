...
public interface ActionItemRepository extends JpaRepository<ActionItem, Integer> {
	...
	// @Query(value = "select * from actionitem_tab where user_id=:emailId AND
	// (actionitem_title_title='%:actionItemTitle%' OR task_priority=:taskPriority
	// OR start_date>=:startDate OR due_date<=:dueDate)", nativeQuery = true)
	// @Query("FROM ActionItem WHERE emailId=:emailId AND (actionItemTitle LIKE
	// %:actionItemTitle% OR :actionItemOwner IS NULL OR startDate<=:startDate OR
	// endDate<=:endDate)")
	@Query(value = "SELECT a.* FROM actionitem_tab a " + "JOIN action_item_owners o ON o.action_item_id = a.id "
			...
			+ "(CAST(:endDate AS DATE) IS NULL OR a.start_date <= :endDate )" + "GROUP BY a.id", nativeQuery = true)
